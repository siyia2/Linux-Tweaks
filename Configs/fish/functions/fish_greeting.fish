...
echo Recursively Chown: (set_color green --bold)'"chown -R USER:GROUP FILE"'(set_color normal) Recursively RW: (set_color green --bold)'"chmod -R 775 FILE"'(set_color normal) Make Executable: (set_color green --bold)'"chmod +x FILE"'(set_color normal)
echo " "
end 
