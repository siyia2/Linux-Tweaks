...
void rename_path(const fs::path& path, const std::string& case_input, bool rename_parents, bool verbose = true) {
    ...
    for (const auto& entry : fs::directory_iterator(path)) {
        ...
        if (entry.is_directory()) {
            if (threads.size() < max_threads) {
                threads.emplace_back(rename_directory, entry.path(), case_input, rename_parents, verbose);
            } else {
                rename_directory(entry.path(), case_input, rename_parents, verbose);
            }
    ...
    for (const auto& path : paths) {
        ...
        if (fs::exists(current_path)) {
            if (fs::is_directory(current_path)) {
                rename_path(current_path, case_input, rename_parents, true);
